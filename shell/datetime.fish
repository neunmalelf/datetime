# Fish completion for `datetime` and for its `timestamp` build variant
...
# as both `datetime.fish` and `timestamp.fish`.
#
...
#   cp shell/datetime.fish ~/.config/fish/completions/timestamp.fish

for cmd in datetime timestamp
...
# Disable file completion for date strings (-d, -s) to avoid misleading suggestions
# (already -f above gives -r -f which suppresses file fallback, but explicitly
#  no file completion needed).  The above -r -f ensures fish does not fall back
#  to files for those options.

# +FORMAT handling: fish completes files by default after options; for +FORMAT
# we silence that (format strings are arbitrary).  No additional action needed;
# the user typing +FORMAT will see no file suggestions because we marked
# most options as -f (no file).  The positional after options will still offer
# file completion, which is fine for -r operands and MMDDhhmm set-time operands
