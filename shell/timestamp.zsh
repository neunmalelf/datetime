#compdef timestamp datetime
# Zsh completion for `timestamp` and for its `datetime` build variant
...
# as `_timestamp` (and `datetime.zsh` as `_datetime`).
#
# User-local install without system zsh completions, add to ~/.zshrc:
#   fpath=(~/.local/share/zsh/site-functions $fpath)
#   autoload -Uz compinit && compinit
# or source directly:
#   source ~/.local/share/zsh/site-functions/_timestamp
#   source shell/timestamp.zsh  # from repo
...
# Register for both commands (datetime and timestamp share the same options)
if (( $+functions[compdef] )); then
    compdef _timestamp timestamp datetime 2>/dev/null || compdef _timestamp timestamp
fi
