...
#
# Bash completion for `datetime` and for its `timestamp` build variant
...
# as both `datetime` and `timestamp`.
#
# User-local install without bash-completion, source it from ~/.bashrc:
#   [ -f ~/.local/share/bash-completion/completions/datetime ] && \
#       . ~/.local/share/bash-completion/completions/datetime

# _datetime_complete CMD CUR PREV
_datetime_complete ()
{
    local cur=$2
    local prev=$3
