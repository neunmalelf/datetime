# Archived: Implementation Plan: datetime Optimization, Simplification & Portability

> **Archive:** Fully implemented and verified 2026-09-12 (build `20260912134149Z`) — all 17 tasks checked, `make check` rc=0, bash 139 + 284, pytest 40 tests / 244 subtests, zero warnings. Supersedes [`history/todo_completed_20260909083118Z.md`](todo_completed_20260909083118Z.md).
...
- [x] **Step 2: Run both bash test suites**
  ...
  - Expected: `Summary: 126 passed, 0 failed, 126 total` (122 baseline + 4 timestamp tests from Task 4).
...
### Task 16: Makefile GNU Convention Polish (suggestions_agy.md 2.5, 2.7, 2.8, 2.9, 2.10)

**Files:**
- Modify: [`Makefile:14-17, 40-46, 55-60, 62-70, 130-155`](Makefile)

- [x] **Step 1: Define `MAKEINFO` / `TEXI2DVI` variables**
  - They were used by `info`/`dvi` but never defined, producing confusing
    "command not found" failures. Now defined next to `INSTALL`.

- [x] **Step 2: Standardize Windows/OS detection**
  - Single `ifeq ($(OS),Windows_NT)` block sets `TARGET ?= datetime` /
    `TARGET2 ?= timestamp` via an `else` branch; the redundant
    `ifndef OS` block was removed.

- [x] **Step 3: Replace `PREFIX ?= $(HOME)/sbin` + string-equality override with GNU convention**
  - `prefix` stays `/usr/local`; `PREFIX` (if set) is forwarded to `prefix`.
  - Users install locally with `PREFIX=$$HOME make install`
    (binaries in `~/bin`, man page in `~/.local/share/man/man1`
    via the now-standard `$(prefix)/share/man` default).

- [x] **Step 4: Remove failure-hiding `-` prefixes from `install`**
  - Man-page `$(INSTALL_DATA)` lines no longer swallow errors;
    `-` remains only on uninstall/legacy cleanup.

- [x] **Step 5: GNU-standard directory defaults**
  - `mandir = $(prefix)/share/man`, `infodir = $(prefix)/share/info`.

- [x] **Step 6: Verify install / uninstall / dist**
  - `make install DESTDIR=... prefix=...` places binaries in `<prefix>/bin`
    and the man page in `<prefix>/share/man/man1`; `make uninstall` removes all;
    `make dist` unchanged (13 files).

### Task 17: Coverage Tests, Valgrind Error Paths & CI (suggestions_agy.md 3.4, 3.6)

**Files:**
- Modify: [`Makefile`](Makefile) (`check-mem`)
- Modify: [`tests/_test_datetime`](tests/_test_datetime)
- Create: [`.github/workflows/ci.yml`](.github/workflows/ci.yml)

- [x] **Step 1: Valgrind error-path coverage**
  - `check-mem` now also runs `--bogus-flag`, bad date, missing `-r` file,
    and mutually-exclusive-option invocations under valgrind with a
    distinct `--error-exitcode=99` (client rc 0/1 expected).

- [x] **Step 2: New behavioral tests in `_test_datetime` (+13 checks)**
  - `shell_escape`: backtick and `$( )` injection payloads must not execute
    (payloads passed via `env` so the harness itself never evaluates them).
  - `format_time`: `%q`, `%z`, `%:z`, `%::z`, `%:::z`, `%N` on `-u` epochs.
  - `-r FILE`: sub-second `%N` path; empty reference path rejected.
  - `-f -`: multi-line stdin; trailing-whitespace tolerance.
  - Unknown `--opt=value` form rejected.
  - Suite grew 126 -> 139, all passing.

- [x] **Step 3: CI workflow (3 OS matrix)**
  - `linux`: gcc + clang, zero-warning build, `check-syntax`, `check-mem`,
    both bash suites, pytest, `make dist` sanity.
  - `macos`: clang build + checks (valgrind-skip path exercised).
  - `windows`: MSYS2/MinGW-w64 build + `check-syntax` + pytest
    (compiles the `getopt.h`, `_mkgmtime`, `_timezone` paths from Task 3;
    bash suites need GNU coreutils and run on Linux/macOS only).

- [x] **Step 4: Full battery green**
  - `make check` rc=0; bash 139 + 284; pytest 40 tests / 244 subtests.
...
graph TD
    ...
    T14 --> T15
    T15 --> T16[Task 16: Makefile GNU Conventions]
    T16 --> T17[Task 17: Coverage Tests & CI]
```
